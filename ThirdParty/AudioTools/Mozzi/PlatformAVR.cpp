...
void PlatformAVR::setAudioRate(uint16_t rate){
    audio_rate = rate;
}
...
void PlatformAVR::start(int control_rate_hz) {
  setupMozziADC(); 
...
void PlatformAVR::stop() {
  stopTimers();
}

unsigned long PlatformAVR::audioTicks() {
  return PlatformCommon::audioTicks();
}

unsigned long PlatformAVR::mozziMicros() { 
  return PlatformCommon::mozziMicros();;
}

void PlatformAVR::audioHook()  {
  PlatformCommon::audioHook();
}

bool PlatformAVR::canWrite(){
  return canBufferAudioOutput();
}

bool PlatformAVR::write(const AudioOutput out){
  return Common.write(out);
}

#if (USE_AUDIO_INPUT == true)
int PlatformAVR::getAudioInput() {
