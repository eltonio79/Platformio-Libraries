...
@note This option superseeds the earlier OUTPUT_STEREO_HACK, which is still available at
      the time of this writing, but should not be used in new sketches.

@note At the time of this writing, only OUTPUT_MONO and OUTPUT_STEREO are supported. The value of
      OUTPUT_MONO is 1 and the value of OUTPUT_STEREO is 2, so future extensions are also expected
      to set this to the number of available channels. */
#define AUDIO_CHANNELS OUTPUT_MONO
//#define AUDIO_CHANNELS OUTPUT_STEREO
...
/** @ingroup core
 The Mozzi guts has been replaced by separate processor specific implementation files. You can switch back to the old implementation by setting 
 USE_LEGACY_GUTS to true in hardware_defines.h - This has no impact for the new implementations (e.g. RPI or MBED). Currently we use USE_LEGACY_GUTS
 true as default values for most of the environments. 
 */
