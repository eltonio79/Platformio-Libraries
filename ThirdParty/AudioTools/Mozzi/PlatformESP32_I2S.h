...
static uint64_t samples_written_to_buffer=0;
extern PLATFORM_OUTPUT_CLASS Mozzi;

/**
 * @brief Hardware sepcific implementation for ESP32 supporting both binary output to an external DAC and analog output
 * 
 */

class PlatformESP32_I2S : public PlatformOutput {
  ...
  public:
    ...
    bool canWrite(){
      return this->canBufferAudioOutput();
    }

    bool write(const AudioOutput out){
      if (this->canBufferAudioOutput() ){
          this->bufferAudioOutput(out);
          return true;
      } else {
          return false;
      }
