...
- OUTPUT_STEREO not yet implemented (although that should not be too hard to do)
...
- Several audio output modes exist, the default being PDM_VIA_SERIAL (configurable in AudioConfigESP8266.h):
  ...
  - EXTERNAL_DAC_VIA_I2S: Output is sent to an external DAC (such as a PT8211), digitally coded. This is the only mode that supports OUTPUT_STEREO. It also needs the least processing power.
