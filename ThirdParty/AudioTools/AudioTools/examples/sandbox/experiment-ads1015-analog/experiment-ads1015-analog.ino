/**
 * @file streams-adc1015-analog.ino
 * @author Phil Schatzmann
 * @brief Use ads1015 as input device
 * => does not work because the output rate is too small
 * @author Phil Schatzmann
 * @copyright GPLv3
 */
...
void setup(void) {
  ...
  cfg.adapt_sample_rate = true;
  in.setNotifyAudioChange(out);
  in.begin(cfg);
 
  // Output 
  auto cfgo = out.defaultConfig();
  cfgo.sample_rate = sample_rate;
  cfgo.channels = channels;
  cfgo.bits_per_sample = bits_per_sample;
  out.begin(cfgo);
