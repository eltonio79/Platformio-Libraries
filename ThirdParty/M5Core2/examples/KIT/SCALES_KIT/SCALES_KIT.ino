...
* date: 2022/02/23
*******************************************************************************
  Connect WEIGHT UNIT to port A (G32/33), calibration instructions: touch button A to remove the tare weight when there is no load,
  touch button B, switch the standard weight value left and right and put down the corresponding weight, confirm for calibration.
  将WEIGHT UNIT连接至端口A（G32/33）， 校准说明: 无负重情况下触摸按键A去处皮重， 触摸按键B，左右触摸切换标准重量值并放下对应重量砝码，comfirm进行校准。
  Libraries:
