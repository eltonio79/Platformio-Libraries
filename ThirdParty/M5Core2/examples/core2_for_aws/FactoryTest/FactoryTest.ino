/*
    ...
    1) ArduinoECCX08:
        ...
        - Location of the M5Core2 Arduino library --> examples --> core2_for_aws
   --> ArduinoECCX08.zip 2) FastLED v3.4.0:
...
int checkPsram() {
    ...
    if (testbuff == nullptr) {
        ...
        CoverScrollText("PSRAM malloc Successful",
                        M5.Lcd.color565(SUCCE_COLOR));
    }
    delay(100);
...
int checkAETCC608AInit() {
    if (!myECCX08.begin()) {
        ...
        CoverScrollText("AT608A Check Successful",
                        M5.Lcd.color565(SUCCE_COLOR));
    }
...
void powerSetup() {
    ...
    DisPowerbuff.drawColorBitmap(0, 39, 77, 24, (uint8_t *)image_power,
                                 0xff9c00, 0x000000);
...
void choosePower() {
    ...
    DisPowerbuff.drawColorBitmap(0, 0, 77, 37, (uint8_t *)image_Internal,
                                 color1, 0x000000);

    DisPowerbuff.drawColorBitmap(0, 65, 77, 37, (uint8_t *)image_External,
                                 color2, 0x000000);

    DisPowerbuff.drawColorBitmap(54, posy, 20, 31, (uint8_t *)image_PowerC,
                                 0xae2828, 0x000000);
...
void Polar2Cartesian(double Angle, double radius, point_2d_t *point,
                     int16_t offsetX = 42, int16_t officeY = 28) {
...
void MPU6886Test() {
    ...
    for (int n = 0; n < 12; n++) {
        line3d.RotatePoint(&rect_source[n].start_point, &rect_dis.start_point,
                           theta, phi, (double)0);
...
void clockFlush() {
    ...
    DisClockbuff.fillRect(0, 20, 162, 50,
                          DisClockbuff.color565(0x1a, 0x11, 0x00));
    DisClockbuff.fillRect(0, 25, 2, 45,
                          DisClockbuff.color565(0xff, 0x9c, 0x00));

    DisClockbuff.fillRect(56, 35, 5, 5,
                          DisClockbuff.color565(0xff, 0x9c, 0x00));
    DisClockbuff.fillRect(56, 54, 5, 5,
                          DisClockbuff.color565(0xff, 0x9c, 0x00));

    DisClockbuff.fillRect(118, 40, 5, 5,
                          DisClockbuff.color565(0xff, 0x9c, 0x00));
    DisClockbuff.fillRect(118, 57, 5, 5,
                          DisClockbuff.color565(0xff, 0x9c, 0x00));
    ...
    DisClockbuff.drawColorBitmap(
        0, 0, 164, 17, rectptrBuff[sytState.clockCount], 0xff9c00, 0x000000);
...
void batpowerFlush() {
    ...
    DisBatbuff.drawColorBitmap(3, 37, 60, 10, Number_7x10px[14], color,
                               0x000000);
    ...
    DisBatbuff.drawColorBitmap(3, 20, 7, 10, Number_7x10px[batVoltage1],
                               0xff9c00, 0x000000);

    DisBatbuff.drawColorBitmap(13, 20, 7, 10, Number_7x10px[batVoltage2],
                               0xff9c00, 0x000000);

    DisBatbuff.drawColorBitmap(20, 20, 7, 10, Number_7x10px[10], 0xff9c00,
                               0x000000);
    ...
    DisBatbuff.drawColorBitmap(31, 19, 27, 11, batRect[sytState.batCount],
                               0xff9c00, 0x000000);
    ...
    if (sytState.batVoltageWriteCount > 50) {
        ...
        sytState.batVoltageBuff[sytState.batVoltageWriteptr] =
            (batPercentage / 20) + 1;
        ...
        DisBatbuff.fillRect(6, 50, 55, 29,
                            DisBatbuff.color565(0x33, 0x20, 0x00));

        for (int i = 0; i < 11; i++) {
            int lim =
                sytState.batVoltageBuff[(sytState.batVoltageReadptr + i) % 15];
            for (int y = 0; y < lim; y++) {
...
void touchsetup() {
    ...
    DisTouchbuff.drawColorBitmap(12, 15, 7, 10, (uint8_t *)Number_7x10px[11],
                                 0xff9c00, 0x000000);
    DisTouchbuff.drawColorBitmap(19, 15, 3, 10, (uint8_t *)Number_7x10px[13],
                                 0xff9c00, 0x000000);

    DisTouchbuff.drawColorBitmap(12, 26, 7, 10, (uint8_t *)Number_7x10px[12],
                                 0xff9c00, 0x000000);
    DisTouchbuff.drawColorBitmap(19, 26, 3, 10, (uint8_t *)Number_7x10px[13],
                                 0xff9c00, 0x000000);
    for (int i = 0; i < 3; i++) {
        int number = (0 / (int)(pow(10, (2 - i)))) % 10;
        DisTouchbuff.drawColorBitmap(22 + i * 7, 15, 7, 10,
                                     (uint8_t *)Number_7x10px[number], 0xff9c00,
                                     0x000000);
    }
...
void touchflush() {
    ...
    if (touchStateNow) {
        ...
        DisTouchbuff.drawColorBitmap(
            12, 15, 7, 10, (uint8_t *)Number_7x10px[11], 0xff9c00, 0x000000);
        DisTouchbuff.drawColorBitmap(
            19, 15, 3, 10, (uint8_t *)Number_7x10px[13], 0xff9c00, 0x000000);

        DisTouchbuff.drawColorBitmap(
            12, 26, 7, 10, (uint8_t *)Number_7x10px[12], 0xff9c00, 0x000000);
        DisTouchbuff.drawColorBitmap(
            19, 26, 3, 10, (uint8_t *)Number_7x10px[13], 0xff9c00, 0x000000);
        for (int i = 0; i < 3; i++) {
...
void sdcardSetup() {
    ...
    if (Type == CARD_UNKNOWN || Type == CARD_NONE) {
        ...
        DisSDCardbuff.drawColorBitmap(
            3, 19, 7, 10, (uint8_t *)Number_7x10px[sdcardFreeSize / 100 % 10],
            0xff9c00, 0x000000);
        DisSDCardbuff.drawColorBitmap(
            10, 19, 7, 10, (uint8_t *)Number_7x10px[sdcardFreeSize / 10 % 10],
            0xff9c00, 0x000000);
        DisSDCardbuff.drawColorBitmap(
            20, 19, 7, 10, (uint8_t *)Number_7x10px[sdcardFreeSize % 10],
            0xff9c00, 0x000000);

        DisSDCardbuff.drawColorBitmap(
            33, 19, 7, 10, (uint8_t *)Number_7x10px[sdcardSize / 100 % 10],
            0xff9c00, 0x000000);
        DisSDCardbuff.drawColorBitmap(
            40, 19, 7, 10, (uint8_t *)Number_7x10px[sdcardSize / 10 % 10],
            0xff9c00, 0x000000);
...
void AppOut() { Serial.println("Hello App"); }
...
void drawJPEGImageFromSDCard(fs::FS &fs, const char *dirname) {
    ...
    while (1) {
        ...
        if ((menuState == 1) || (menuState == 3) || (flushJPEG == true)) {
            ...
            Disbuff.pushInSprite(&menubuff, 0, 0, 320, 34, 0,
                                 241 - menuPosyNow);
            Disbuff.pushSprite(0, 0);
        }
    }
...
void AppWifi() {
    ...
    Disbuff.setFreeFont(&EVA_20px);
    Disbuff.setTextSize(1);
    Disbuff.setTextColor(Disbuff.color565(0xff, 0xa0, 0),
                         Disbuff.color565(0, 0, 0));
    ...
    while (1) {
        ...
        if (WifiNumber == -2) {
            ...
            if (first) {
                ...
                if (count >= 10) {
                    ...
                    Disbuff.drawColorBitmap(122, 98, 76, 44,
                                            TouchFishBuff[count_n], 0xff9c00,
                                            0x000000);
                    Disbuff.pushSprite(0, 0);
                    count_n++;
            ...
            if (first) {
                for (int i = 0; i < WifiNumber; i++) {
                    for (int n = 9; n >= 0; n--) {
                        Disbuff.fillRect(298 - 12 * n, 34 + i * 24 + 2, 8, 20,
                                         color_gery);
                    }
                }
                Disbuff.pushSprite(0, 0);
...
void AppTimer() {
    ...
    uint16_t colorBar[12] = {color16green,  color16green,  color16green,
                             color16green,  color16green,  color16green,
                             color16Yellow, color16Yellow, color16Yellow,
                             color16Yellow, color16Red,    color16Red};
    ...
    while (1) {
        ...
        for (int i = 0; i < 12; i++) {
            if (i < timerect)
                TimerRectbuff.fillRect(0, 154 - i * 14, 30, 10,
                                       colorBar[11 - i]);
            else
        ...
        Timerbuff.drawColorBitmap(posx[0], 4, 24, 42, DigNumber[min / 10],
                                  colorNum, 0x000000);
        Timerbuff.drawColorBitmap(posx[1], 4, 24, 42, DigNumber[min % 10],
                                  colorNum, 0x000000);
        Timerbuff.drawColorBitmap(posx[2], 4, 24, 42, DigNumber[sec / 10],
                                  colorNum, 0x000000);
        Timerbuff.drawColorBitmap(posx[3], 4, 24, 42, DigNumber[sec % 10],
                                  colorNum, 0x000000);
        Timerbuff.drawColorBitmap(
            posx[4], 14, 18, 35, DigNumber_35px[mill / 10], colorNum, 0x000000);
        Timerbuff.drawColorBitmap(
            posx[5], 14, 18, 35, DigNumber_35px[mill % 10], colorNum, 0x000000);
    ...
    while (1) {
        ...
        if ((pos.x != -1) && (pressed == false)) {
            ...
            if (SoundBtn.inHotZone(pos)) {
                sytState.soundFlag =
                    (sytState.soundFlag == true) ? false : true;
...
void setCheckState(int number, bool state, bool flush) {
    ...
    Disbuff.drawColorBitmapAlpha(posx, posy, 53, 33, (uint8_t *)ptr2, color,
                                 0x000000);

    Disbuff.drawColorBitmapAlpha(posx, posy + 1, 53, 33, (uint8_t *)ptr1,
                                 0xff9a00, color);
